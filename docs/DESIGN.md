...
| `bill` | 账单 | house_id（FK，级联）、fee_item_id（FK，级联）、period（账期）、original_amount、adjust_amount、adjust_reason、amount_receivable、amount_received、status（未缴/部分缴纳/已缴清），**唯一**：house_id+fee_item_id+period |
...
4. **软删除 vs 硬删除**：业务数据采用**硬删除 + 级联**（删除前二次确认 + 操作日志留痕）；而"住户变动"不删数据——通过 `resident_house.is_current/end_date` 实现**历史留痕**（搬出、退租、过户均只关闭当前关系并新增/清空，历史轨迹可完整回溯）。
...
1. **账单生成**：金额 = 单价 × 建筑面积 × 月数（月/季/年 = 1/3/12）；按户固定 = 固定金额/期；按车位 = 固定金额 × 该房屋名下车位数（未登记车辆的房屋不生成）。应收不足 0.01 元不生成。
